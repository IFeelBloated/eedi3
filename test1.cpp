...
static inline void vs_bitblt16(void *dstp, int dst_stride, const void *srcp, int src_stride, size_t row_size, size_t height) {
	if (height) {
		if (src_stride == dst_stride && src_stride == (int)row_size) {
			memcpy(dstp, srcp, row_size * height);
		}
		else {
			uint16_t *srcp8 = (uint16_t *)srcp;
			uint16_t *dstp8 = (uint16_t *)dstp;
...
static VSFrameRef *copyPad(const VSFrameRef *src, int fn, VSFrameContext *frameCtx, VSCore *core, const VSAPI *vsapi, void **instanceData)
{
    ...
    if(!d->dh) {
        for(b = 0; b < d->vi.format->numPlanes; ++b)
          vs_bitblt16(reinterpret_cast <uint16_t *> (vsapi->getWritePtr(srcPF, b)) + (vsapi->getStride(srcPF, b)/2) * (4 + off) + 12,
                      (vsapi->getStride(srcPF, b)/2) * 2,
                      reinterpret_cast <const uint16_t *> (vsapi->getReadPtr(src, b)) + (vsapi->getStride(src, b)/2)*off,
        ...
        for(b = 0; b < d->vi.format->numPlanes; ++b)
          vs_bitblt16(reinterpret_cast <uint16_t *> (vsapi->getWritePtr(srcPF, b)) + (vsapi->getStride(srcPF, b)/2) * (4 + off) + 12,
                      (vsapi->getStride(srcPF, b)/2) * 2,
                      reinterpret_cast <const uint16_t *> (vsapi->getReadPtr(src, b)),
    ...
    for(b = 0; b < d->vi.format->numPlanes; ++b) {
        ...
        for(y = off; y < 4; y += 2)
          vs_bitblt16(dstp + y * dst_pitch, dst_pitch,
...
static const VSFrameRef *VS_CC eedi3GetFrame(int n, int activationReason, void **instanceData, void **frameData, VSFrameContext *frameCtx, VSCore *core, const VSAPI *vsapi)
{
    ...
    if(activationReason == arInitial) {
        ...
        for(b = 0; b < d->vi.format->numPlanes; ++b) {
          ...
          vs_bitblt16(dstp + (1 - field_n)*dpitch, dpitch * 2,
